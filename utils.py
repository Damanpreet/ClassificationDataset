...
def knearest_neighbor(image):
    ...
    '''
    image = cv2.medianBlur(image,5) # median blur the image to reduce the noise.
    ...
    for r1 in range(ht):
        for c1 in range(wd):
            ...
            for ref in cfg.classes:
                dist = eucledian_dis(image[r1][c1], np.array(ref)) # check the nearest class of the pixel
                if dist < best: 
